...
- **CL**=CLAUDE.md存在 / **SC**=単独完結 / **N2**=命名v2.0 / **MD**=モデル§ / **BM**=branch-cleanupトリガー / **BS**=branch-cleanup SKILL / **GV**=governance参照 / **GC**=governance参照が正典と一致 / **NO**=男座員也 / **NE**=Kazuya Oza / **RM**=README / **HL**=haiku旧記述(✓=削除済)
- **XB**=デフォ以外の余分ブランチ数
- private リポは名前をマスク表示（公開リポにコミットされるため）。実名確認はローカルで `--no-mask` 実行

## マトリクス

| # | リポ | def | CL | SC | N2 | MD | BM | BS | GV | GC | NO | NE | RM | HL | XB |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
